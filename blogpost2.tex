...
\item \textbf{Locality:} \pablo{Locality is the assumption that, at any given instant, the values of any particle's properties are independent of any other particle. Intuitively, it says that particles are individual entities that can be understood in isolation of any other particle. 
...
Most famously, Bell's experiment shows that it is possible to have distinct variables $A$ and $B$ that exhibit correlations that cannot be explained by any classical hidden variable, since classical variables are restricted by the principle of locality [LINK to introduction]. \sout{In fact, if we define a variable $\Lambda$ as the entire past of $A$ and $B$, so that if we create $A$ and $B$ and make sure nothing else can interact with them, then we know everything about $\Lambda$. Still, we may observe correlations that do not satisfy the Markov condition and are therefore not classically causal.}
